import { Keypair, PublicKey, SystemProgram, Transaction, TransactionInstruction, VersionedTransaction, Signer, LAMPORTS_PER_SOL, TransactionMessage, Blockhash, AddressLookupTableAccount } from '@solana/web3.js';
import { loadKeypairs } from './createKeys';
import { calculateTokensBoughtPercentage } from './computeLPO';
import { wallet, connection, tipAcct, walletconn, payer } from '../config';
...
async function distributeWSOL(distributeAmt: number, jitoTip: number, steps = 27) {
    ...
    const totalSolRequired: number = Math.floor((distributeAmt/LAMPORTS_PER_SOL) * 27);
    console.log("Distributing", totalSolRequired + " SOL...");
    ...
    while (running) {
        ...
        switch (answer) {
            ...
            case '5':
                ...
                break;
            case '6':
                const initialBuyAmt = prompt('Distribute amount to each wallet (Ex: 0.3): ');
