import {
  ...
  import { Keypair, PublicKey, SendOptions, TransactionMessage, LAMPORTS_PER_SOL, Connection, Signer, VersionedTransaction, ComputeBudgetProgram, Transaction, SystemProgram, TransactionInstruction, Blockhash } from '@solana/web3.js';
  import {
    connection,
    wallet,
    tipAcct
...
import { pool } from "./clients/poolKeysReassigned";
...
async function makeCreateMarketInstructionSimple<T extends TxVersion>({
  ...
  async function openInEdge(txid: string): Promise<void> {
    return new Promise((resolve, reject) => {
      exec(`start msedge https://solscan.io/tx/${txid}`, (err) => {
        if (err) {
          console.error('Failed to open Edge:', err);
          reject(err);
        } else {
          console.log(`Edge launched with URL: https://solscan.io/tx/${txid}`);
          resolve();
        }
      });
    });
